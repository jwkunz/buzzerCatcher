...
private:
  ...
  const char *state_to_text(GameStateType s)
  {
    switch (s)
    {
    case POWER_UP:
...
public:
            ...
            void *args)
  {
...
#ifdef DEBUG_MODE
              uint8_t n_buffer = min(48, this->MAX_PRINT_STRING_LENGTH);
              char *print_buffer = malloc(n_buffer);
              if (print_buffer == NULL)
              {
                (this->error_handler)("Out of memory");
              }
              snprintf(print_buffer, n_buffer, "Chose response on trigger %u", i + 1);
    ...
    if (next_state != current_state)
    {
      ...
      snprintf(print_buffer, n_buffer, "Transition from %s to %s", this->state_to_text(current_state), this->state_to_text(next_state));
      this->print(print_buffer);
      free(print_buffer);
    }
#endif
